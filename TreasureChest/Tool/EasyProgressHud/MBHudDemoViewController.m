...
@interface MBHudDemoViewController () <NSURLSessionDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.examples =
    @[@[[MBExample exampleWithTitle:@"非自动消失" selector:@selector(reloadMethod1)],
        [MBExample exampleWithTitle:@"自动消失" selector:@selector(reloadMethod_autoDisapper)],
        [MBExample exampleWithTitle:@"请求成功" selector:@selector(reloadMethod_success)],
        [MBExample exampleWithTitle:@"请求失败" selector:@selector(reloadMethod_fail)],
        [MBExample exampleWithTitle:@"请求进度" selector:@selector(reloadMethod_progress)]],
...
- (void)reloadMethod1 {
    [EasyProgress showLoading:@"大师傅的"];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 2.0 * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void){
        [EasyProgress hide];
    });
}

- (void)reloadMethod_autoDisapper {
    [EasyProgress showMessage:@"自动消失"];
//    [EasyProgressHud showMessage:@"自动消失" delay:0.8];
}

- (void)reloadMethod_success {
    [EasyProgress showSuccess:@"请求成功"];
//    [EasyProgressHud showSuccess:@"请求成功" delay:0.8];
}

- (void)reloadMethod_fail {
    [EasyProgress showFail:@"请求失败"];
//    [EasyProgressHud showFail:@"请求失败" delay:0.8];
}
