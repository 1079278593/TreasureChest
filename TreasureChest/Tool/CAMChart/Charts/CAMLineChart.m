...
- (void)calculationYLabels {
    ...
    for (NSInteger i = 0; i < self.chartProfile.xyAxis.yLabelDefaultCount; i++) {
        [arrLabels addObject: [NSString stringWithFormat:@"%.0f",_minTruncValue + yStepValue * i]];
    }
...
- (void)calculationChartCanvas {
    ...
    if(self.chartProfile.xyAxis.showYLabel){
        if([self.yLabels count]){
            ...
            //以最大的Y轴标签宽度作为占位基准宽度
            CGFloat maxValue = [[yLabelWidths valueForKeyPath:@"@max.floatValue"] intValue];//修改类型
...
#pragma mark 将数据映射点转换为绘图路径，可能包括多条数据线。@return array of UIBezierPath
...
#pragma mark 折线类型路径。@return array of UIBezierPath
...
#pragma mark 曲线类型路径。@return array of UIBezierPath
