...
- (UILabel *)titleLabel
{
    if (!_titleLabel) {
        ...
        CGFloat width = KScreenWidth - x - self.selectBtn.x;
...
{
    if (!_lineView) {
        self.lineView = [[UIView alloc] initWithFrame:CGRectMake(12, 0, KScreenWidth - 12, 0.5)];
...
- (UIImageView *)iconImageView
{
    if (!_iconImageView) {
        ...
        _iconImageView.right = KScreenWidth - 14;
    }
...
- (void)layoutSubviews
{
    ...
    self.btn.frame = CGRectMake(0, 0, KScreenWidth * 0.5, self.height);
}
...
- (void)setMenuItem:(CollapsibleModel *)menuItem
{
    ...
    self.lineView.x = x - 1;
    self.lineView.width = KScreenWidth - self.lineView.x - 12;
}
