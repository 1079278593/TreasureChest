...
- (void)removeFormDirection:(DraggableDirection)direction {
    ...
    if (self.moving) {
        ...
        switch (direction) {
            case DraggableDirectionLeft:
                cardCenter = CGPointMake(-CCWidth / 2, self.cardCenter.y);
                flag = -1;
                break;
            case DraggableDirectionRight:
...
- (void)tapGestureHandle:(UITapGestureRecognizer *)tap {
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:didSelectIndex:)]) {
        [self.delegate draggableContainer:self cardView:(DraggableCardView *)tap.view didSelectIndex:tap.view.tag];
    }
}
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if ([self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            //  ---------------------------------------------------------------------------------------------
            ...
            [self judgeMovingState: widthRatio];    // Moving
            
            //  ----------------------------------------
            //  左右的判断方法为: 只要 ratio_w > 0 就是Right
            //  ----------------------------------------
            if (widthRatio > 0) {
                self.direction = DraggableDirectionRight;
            } if (widthRatio < 0) {
                self.direction = DraggableDirectionLeft;
            } else if (widthRatio == 0) {
                self.direction = DraggableDirectionDefault;
            }
...
//  移除最底层Card
//  ---------------
- (void)finishedPanGesture:(UIView *)cardView direction:(DraggableDirection)direction scale:(CGFloat)scale disappear:(BOOL)disappear {
    if (!disappear) {
        ...
        // 2.重新布局剩下的cardViews
        // --------------------------------
        NSInteger flag = direction == DraggableDirectionLeft ? -1 : 2;
...
- (void)movingVisibleCards:(CGFloat)scale {
    ...
    for (int i = 1; i < self.currentCards.count; i++) {
        DraggableCardView *cardView = [self.currentCards objectAtIndex:i];
        switch (i) {
            case 1:
            {
    ...
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfIndexs)] && [self.dataSource respondsToSelector:@selector(draggableContainer:viewForIndex:)]) {
        ...
        if (self.loadedIndex < indexs) {
            for (long int i = self.currentCards.count; i <  (self.moving ? preloadViewCont + 1: preloadViewCont); i++) {
                DraggableCardView *cardView = [self.dataSource draggableContainer:self viewForIndex:self.loadedIndex];
                cardView.frame = CGRectMake(kContainerEdage, kContainerEdage,
                                            self.frame.size.width  - kContainerEdage * 2,
                                            self.frame.size.height - kContainerEdage * 2);
        ...
        NSAssert(self.dataSource, @"DraggableContainerDataSource can't nil");
    }
}
...
- (void)resetVisibleCards {
    __weak DraggableContainer *weakself = self;
    [UIView animateWithDuration:0.5 delay:0.0 usingSpringWithDamping:0.6 initialSpringVelocity:0.0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction animations:^{
