...
artnet_node artnet_new(const char *ip, int verbose) {
  ...
  // now setup the default parameters：现在设置默认参数
  ...
  // set all ports to MERGE HTP mode and disable：将所有端口设置为MERGE http模式并禁用
...
int artnet_start(artnet_node vn) {
  ...
  // build the initial reply: 构建初始回复
  if ((ret = artnet_tx_build_art_poll_reply(n)))
    return ret;

  if (n->state.node_type == ARTNET_SRV) {//server
    // poll the network
