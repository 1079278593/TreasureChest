...
#import "PTExampleProtocol.h"   //!< 在此文件夹内：Peertalk macOS Example
...
// didAcceptConnection后，发送一条’本机系统配置‘的消息。
- (void)sendDeviceInfo {
  if (!peerChannel_) {
    return;
  }
...
- (BOOL)ioFrameChannel:(PTChannel*)channel shouldAcceptFrameOfType:(uint32_t)type tag:(uint32_t)tag payloadSize:(uint32_t)payloadSize {
    NSLog(@"peer talk shouldAcceptFrameOfType");
...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(NSData *)payload {
    NSLog(@"peer talk didReceiveFrameOfType");
...
- (void)ioFrameChannel:(PTChannel*)channel didEndWithError:(NSError*)error {
    NSLog(@"peer talk didEndWithError");
  if (error) {
...
- (void)ioFrameChannel:(PTChannel*)channel didAcceptConnection:(PTChannel*)otherChannel fromAddress:(PTAddress*)address {
    NSLog(@"peer talk didAcceptConnection");
