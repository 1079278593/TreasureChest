...
#define KHttpRequestTimeout 3.5     //超时时间，超过这个时间没响应就要处理
...
- (void)connectWithHost:(NSString *)host port:(NSUInteger)port {
    if (![_host isEqualToString:host]) {
        [self.socket disconnect];//host不同，断开！（升级和控制界面的host不一定相同，相同时直接使用，不同时断开。）
    }
    ...
    if (![self.socket isConnected]) {
        NSError *error = nil;
        [self.socket connectToHost:_host onPort:_port withTimeout:100000 error:&error];
        if (error != nil) {
...
- (void)disconnect {
//    NSLog(@"httpReqeust: 断开命令 %s", __func__);
...
- (void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port {
    ...
    NSLog(@"httpRequest：socket连接成功：%d",self.isDisconnectInner);
}

- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
    NSLog(@"httpReqeust: socket已断开 %@ %@", self.currentRequest.url, err);
...
//        NSLog(@"3434343：-2 inner：%d",self.isDisconnectInner);
    }
...
#pragma mark - < process >
///通知：连接、断开
- (void)isSocketConnected:(BOOL)isConnect {
...
- (void)processDelayRequest {
    if (self.queue.count > KHttpRequestDelayCount) {
        ...
        NSLog(@"httpRequestQueue，积压超过：%lu个(有断开操作)",self.queue.count);
...
//    NSLog(@"httpQueue: process next request:%lu",self.queue.count);
    ...
    [self.content setLength:0];
    //调整timeout时间可能可以解决，最初1秒，待测验（Error Domain=GCDAsyncSocketErrorDomain Code=4 "Read operation timed out" ）
    [self.socket writeData:[httpRequestHeader dataUsingEncoding:NSUTF8StringEncoding] withTimeout:KHttpRequestTimeout tag:GCD_TAG_WRITE_REQUEST];
}
...
- (void)addHttpRequest:(HttpRequest*)request {
    ...
    NSLog(@"httpQueue: add request, request总数量:%lu",self.queue.count);
}
