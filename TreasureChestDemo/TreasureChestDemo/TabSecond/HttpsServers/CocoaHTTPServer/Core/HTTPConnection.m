...
//    NSString *thePath = [[NSBundle mainBundle] pathForResource:@"imemorys" ofType:@"p12"];
    NSString *thePath = [[NSBundle mainBundle] pathForResource:@"Web" ofType:@""];
    thePath = [NSString stringWithFormat:@"%@/imemorys.p12",thePath];
...
- (void)replyToHTTPRequest
{
	...
	if ([WebSocket isWebSocketRequest:request])
	{
		...
		else
		{
			...
			{
				NSLog(@"%@[%p]: WebSocket forgot to set itself as socket delegate", THIS_FILE, self);
...
- (NSString *)filePathForURI:(NSString *)path allowDirectory:(BOOL)allowDirectory
{
	...
	if (documentRoot == nil)
	{
		NSLog(@"%@[%p]: No configured document root", THIS_FILE, self);
		return nil;
	}
	...
	if (docRoot == nil)
	{
		NSLog(@"%@[%p]: Document root is invalid file path", THIS_FILE, self);
		return nil;
	}
	...
	{
		NSLog(@"%@[%p]: Request for file outside document root", THIS_FILE, self);
		return nil;
	}
...
- (void)responseHasAvailableData:(NSObject<HTTPResponse> *)sender
{
	...
	dispatch_async(connectionQueue, ^{ @autoreleasepool {
		...
		{
			NSLog(@"%@[%p]: %@ - Sender is not current httpResponse", THIS_FILE, self, THIS_METHOD);
			return;
		}
