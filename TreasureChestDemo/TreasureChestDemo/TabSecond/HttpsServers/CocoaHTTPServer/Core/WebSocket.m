...
+ (BOOL)isWebSocketRequest:(HTTPMessage *)request
{
    ...
    NSLog(@"%@: %@ - %@", THIS_FILE, THIS_METHOD, (isWebSocket ? @"YES" : @"NO"));
...
+ (BOOL)isVersion76Request:(HTTPMessage *)request
{
    ...
    NSLog(@"%@: %@ - %@", THIS_FILE, THIS_METHOD, (isVersion76 ? @"YES" : @"NO"));
...
+ (BOOL)isRFC6455Request:(HTTPMessage *)request
{
    ...
    NSLog(@"%@: %@ - %@", THIS_FILE, THIS_METHOD, (isRFC6455 ? @"YES" : @"NO"));
...
- (NSData *)processKey:(NSString *)key
{
    ...
    NSLog(@"key(%@) -> %qi / %qi = %qi", key, num, numSpaces, resultHostNum);
...
- (void)sendResponseBody:(NSData *)d3
{
	...
	if (HTTP_LOG_VERBOSE)
	{
        ...
        NSLog(@"key1 result : raw(%@) str(%@)", d1, s1);
        NSLog(@"key2 result : raw(%@) str(%@)", d2, s2);
        NSLog(@"key3 passed : raw(%@) str(%@)", d3, s3);
        NSLog(@"key0 concat : raw(%@) str(%@)", d0, s0);
        NSLog(@"responseBody: raw(%@) str(%@)", responseBody, sH);
...
{
    NSLog(@"%@[%p]: socketDidDisconnect:withError: %@", THIS_FILE, self, error);
