		...
		if( checkForContentEnd ) {
			if(	pendingData.length >= 2 ) {
				if( *(uint16_t*)(pendingData.bytes + offset) == 0x2D2D ) {
                    ...
                    NSLog(@"MultipartFormDataParser: End of multipart message");
					waitingForCRLF = YES;
					reachedEpilogue = YES;
					offset+= 2;
				}
	...
	while( true ) {
		...
		if( nil == currentHeader ) {
			...
			if( -1 == headerEnd ) {
                ...
                NSLog(@"MultipartFormDataParser: MultipartFormDataParser: Retrieved part header.");
			}
		...
		if( contentEnd == -1 ) {
			...
			if( [delegate respondsToSelector:@selector(processContent:WithHeader:)] ) {
                NSLog(@"MultipartFormDataParser: Processed %d bytes of body",sizeToPass);
			...
			if( [delegate respondsToSelector:@selector(processEndOfPartWithHeader:)] ){
                ...
                NSLog(@"MultipartFormDataParser: End of body part");
			}
			currentHeader = nil;
	...
	if( offset == dataLength ) {
		...
		if( sizeToProcess > 0) {
			if( [delegate respondsToSelector:@selector(processPreambleData:)] ) {
                ...
                NSLog(@"MultipartFormDataParser: processed preamble");
			}
