	...
	while (offset < stopOffset)
	{
		...
		if (memcmp(subBuffer, sep, sepLen) == 0)
		{
			if (!found1)
			{
				...
				NSLog(@"%@[%p]: Found s1 at %lu", THIS_FILE, self, (unsigned long)s1);
			}
			else
			{
				...
				NSLog(@"%@[%p]: Found s2 at %lu", THIS_FILE, self, (unsigned long)s2);
			}
			...
			if (found1 && found2)
			{
				...
				if (key)
				{
					...
					if (value)
					{
						...
						NSLog(@"%@[%p]: key(%@) -> value(%@)", THIS_FILE, self, key, value);
